...
namespace AMMS.Infrastructure.Repositories
{
    ...
    CancellationToken ct = default)
        ...
        public async Task<AllOrdersProductionTrackingRawResult> GetAllOrdersProductionTrackingRawAsync(int page, int pageSize, CancellationToken ct = default)
        {
            if (page <= 0) page = 1;
            if (pageSize <= 0) pageSize = 10;
            if (pageSize > 200) pageSize = 200;
            ...
            var hasNext = orders.Count > pageSize;
            ...
            if (orders.Count == 0)
            {
                return new AllOrdersProductionTrackingRawResult
                {
                    Page = page,
                    PageSize = pageSize,
                    HasNext = false
            ...
            var requests = await _db.order_requests
                .AsNoTracking()
                .Where(r => r.order_id != null && orderIds.Contains(r.order_id.Value))
                .Select(r => new OrderRequestIdRaw
                {
                    order_request_id = r.order_request_id,
                    order_id = r.order_id
                })
